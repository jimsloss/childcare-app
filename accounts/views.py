...
def accounts(request):
    ...
    if invoicestotal > 999:
        invoicestotal = "£" + str(invoicestotal)[:-3] +"," + str(invoicestotal)[-3:]
    else:
        invoicestotal ="£" + str(invoicestotal)

    for purchase in purchases:
        purchase.month = purchase.date.strftime("%B")
        purchasestotal += purchase.amount

    if purchasestotal > 999:
        purchasestotal = "£" + str(purchasestotal)[:-3] +"," + str(purchasestotal)[-3:]
    else:
        purchasestotal ="£" + str(purchasestotal)
...
def newInvoice(request):
    ...
    childrate["Part Time"] = rates.partday
    childrate["Part Time Retainer"] = rates.partdayretainer
...
def invoiceview(request):
    ...
    #### delete action .. need ref to the invoice
    ...
    total = theInvoice.total
    
    if len(str(total)) >3:
        total = "£"+ str(total)[:-3]+"," + str(total)[-3:]
    else:
        total = "£"+str(total)
...
def addPurchase(request):
    ...
    if task =="addpurchase":
        ...
        datestring = request.POST.get("dateOfPurchase")
        ...
        newPurchase = Purchase(date=date, category=category, name=name, amount = float(payment), purchase = purchase )

        newPurchase.month = newPurchase.date.strftime("%B")
        newPurchase.save()
        
        day = newPurchase.date.strftime("%d")
        year = newPurchase.date.strftime("%Y")
                
        month = int(newPurchase.date.strftime("%m"))

        if month > 4:
            newPurchase.taxyear = year +"/"+ str(int(year)+1)
        elif month == 4 and int(day) >4:
            newPurchase.taxyear = year +"/"+ str(int(year)+1)
        else:
            newPurchase.taxyear = str(int(year)-1) +"/"+ year
...
def changefees(request):
    ...
    if charges:  # adversly, this means if no instances exist
        ...
        charges.saturdaycare = 12
        charges.eveningcare = 15
        charges.weddingnanny = 15
        charges.supervised = 15
        charges.nonsupervised = 10
        charges.cosycorner =10

        charges.save()
        
    choices ={"Full Day":charges.fullday,  "Part Day":charges.parttime, "Late Payment":charges.latepayment, "Late Pickup":charges.latepickup, "Saturday Care":charges.saturdaycare, \
        "Evening Care": charges.eveningcare, "Wedding Nanny": charges.weddingnanny, "Supervised Area": charges.supervised, "Non-Supervised Area": charges.nonsupervised,\
            "Cosy Corner": charges.cosycorner}
...
def changefees2(request):
    ...
    if 'cancel' in request.POST:
        ...
        #id = request.POST["charges"]
        #prices = Pricing.objects.get(pk=id)
        prices = Pricing.objects.first()
        change = request.POST["feechange"]
        amount = request.POST["newfee"]
        
        setattr(prices, change, amount)
    # Saturday Care, Evening Care, Wedding Nanny, Supervised Area, Non-Supervised Area, Cosy Corner
        if change == "Full Day":    
           setattr(prices, "fullday", int(amount))
           # prices.fullday= int(amount)
        elif change == "Part Day":
            setattr(prices, "partday", int(amount))
        elif change == "Late Payment":
            setattr(prices, "latepayment", int(amount))
        elif change == "Late Pickup":
            setattr(prices, "latepickup", int(amount))
        elif change == "Saturday Care":    
            setattr(prices, "saturdaycare", int(amount))
        elif change == "Evening Care":
            setattr(prices, "eveningcare", int(amount))
        elif change == "Wedding Nanny":
            setattr(prices, "weddingnanny", int(amount))
        elif change == "Supervised Area":
            setattr(prices, "supervised", int(amount))
        elif change == "Non-Supervised Area":
            setattr(prices, "nonsupervised", int(amount))
        elif change == "Cosy Corner":
            setattr(prices, "cosycorner", int(amount))
...
def testmonthchange(request):

## copy of accounts view.. temp action ##
