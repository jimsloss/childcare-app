...
class Purchase(models.Model):
    ...
    taxyear=models.CharField(max_length=64,default="")
    ...
    def __str__(self):
        return f"{self.date}: {self.month} - {self.taxyear} - {self.category} - {self.name} - Purchase : {self.purchase} - £ {self.amount}"

class Pricing(models.Model):
    fullday = models.IntegerField(default=0)
    fulldayretainer = models.IntegerField(default=0)
    partday = models.IntegerField(default=0)
    partdayretainer = models.IntegerField(default=0)
    ...
    saturdaycare = models.IntegerField(default=0)
   
    weddingnanny = models.IntegerField(default=0) 
    supervised = models.IntegerField(default=0)
    nonsupervised = models.IntegerField(default=0)
    cosycorner = models.IntegerField(default=0)
      

    def __str__(self):
        return f"fullday: {self.fullday}, Full day retainer: {self.fulldayretainer}, Part day: {self.partday}, \
            Part day retainer: {self.partdayretainer}, Late Payment: {self. latepayment}, Late Pickup: {self. latepickup}, \
                Evening Care: {self.eveningcare},Saturday Care: {self.saturdaycare},Wedding Nanny: {self.weddingnanny}, \
                    Wedding Supervised Area: {self.supervised}, Wedding NonSupervised Area: {self.nonsupervised}, Cosy Corner: {self.cosycorner}"
